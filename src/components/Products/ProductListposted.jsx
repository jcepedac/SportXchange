...
import { ListView, GridView, Search, ProductFilter, Pagination } from "..";
...
const ProductListposted = ({ products }) => {
	...
	const userProducts = filteredProducts.filter(product => product.sellerId === userId);
	//get current product
		...
		<main className="relative">
			<header className="flex flex-col gap-y-4 xl:flex-row xl:items-center justify-between border-b pb-2">
				...
				<div className="flex gap-2 items-center">
					...
					<h1>
						<span className="font-bold">{currentProducts.length} </span>- Products I've
						Uploaded
					</h1>
				</div>
				{/* Search Bar */}
...
export default ProductListposted;
